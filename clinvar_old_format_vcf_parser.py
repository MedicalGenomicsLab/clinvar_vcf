...
def expand_clinvar_vcf(xml_file, vcf_file, out_file):
    ...
    for event, elem in context:
        if elem.tag != 'ClinVarSet' or event != 'end':
            continue 
        rcv_id = elem.find('.//ClinVarAccession').attrib.get('Acc') 
        ms_id = elem.find('.//MeasureSet').attrib.get('ID') 
        cva=elem.findall("./ClinVarAssertion")

        if rcv_id in id_dict:
            xml_dict[id_dict[rcv_id]]['CLNSUBA'] += get_submitters(elem)
            xml_dict[id_dict[rcv_id]]['CLNREVSTATA'] += get_clinsig(cva, field='status_ordered', record_id=ms_id)
            xml_dict[id_dict[rcv_id]]['CLNDATEA'] += get_clinsig(cva, field='last_eval', record_id=ms_id)
            xml_dict[id_dict[rcv_id]]['CLNDATESUBA'] += get_submitdate(cva, field='submitterDate', record_id=ms_id)
            xml_dict[id_dict[rcv_id]]['CLNSIGA'] += get_clinsig(cva, field='description', record_id=ms_id)        
            xml_dict[id_dict[rcv_id]]['CLNORA'] += get_origin(cva, as_set=False) 
            xml_dict[id_dict[rcv_id]]['CLNCOMA'] += get_clinsig(cva, field='comment', record_id=ms_id)  
            
            # Duplicate disease name if multiple SCV entries in one ClinVarSet record
            scv_ids = get_accession(cva, field='SCV')
            xml_dict[id_dict[rcv_id]]['CLNSCVA'] += scv_ids  
            disease_name = '/'.join(get_traits(elem, field='traits'))
            for _ in range(len(scv_ids)):
                xml_dict[id_dict[rcv_id]]['CLNDNA'] += [disease_name]
